      ...
      <Col :span="16" class="g-flex-a-j-center -date-search">
        ...
        <Col span="7" class="g-flex-a-j-center">
          ...
          <div class="-date-search">
            <Button type="primary" class="-p-modal-btn" @click="getSmsMessageList(1)">搜索</Button>
          </div>
        </Col>
      </Col>
    </Row>
  ...
  export default {
    ...
    methods: {
      ...
      getSmsMessageList(num) {
        ...
        if(this.type == '1') {
          param.smsId = this.data
        } else {
          param.taskId = this.data
        }

        this.$api.user.getSmsMessageList(param)
          .then(response => {
