  ...
  export default {
    ...
    methods: {
      ...
      listBookInfoByCompose(data) {
        this.courseListModal = []
        this.$api.hkywhdCompose.listBookInfoByCompose({
          composeId: data.id
        })
          .then(response => {
      ...
      getList() {
        ...
        this.$api.hkywhdCompose.pageCompose({
      ...
      delItem(param) {
        this.$Modal.confirm({
          ...
          content: '确认要删除吗？',
          onOk: () => {
            this.$api.hkywhdCompose.removeCompose({
              composeId: param.id
            }).then(
              response => {
      ...
      changeStatus(data) {
        this.$api.hkywhdCompose.display({
          composeId: data.id
        }).then(
          response => {
            if (response.data.code == "200") {
      ...
      submitInfo(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.isSending = true
            this.$api.hkywhdCompose.saveComposeService({
              ...this.addInfo,
      ...
      submitModalCourse () {
        ...
        this.$api.hkywhdCompose.saveLinkBook({
