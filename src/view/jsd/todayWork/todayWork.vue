    ...
    <Card>
      <Row class="g-search">
        ...
        <Row class="p-todayWork-flex" v-if="radioType === 0">
          <Col :span="4">
            <Card class="-item-wrap">
              ...
              <div class="-item-wrap-center">
                {{ countInfo.total }}/{{ countInfo.totalHandled }}
              </div>
              <div class="-item-wrap-down">
                {{
                  (countInfo.totalHandled / countInfo.total).toFixed(3) * 100
                }}%
              </div>
            </Card>
          </Col>
          ...
          <Col :span="4">
            <Card class="-item-wrap">
              ...
              <div class="-item-wrap-center">
                {{ countInfo.resubmitnum }}/{{ countInfo.handleResubmit }}
              </div>
              <div class="-item-wrap-down">
                {{
                  (countInfo.handleResubmit / countInfo.resubmitnum).toFixed(
                    3
                  ) * 100
                }}%
              </div>
        ...
        <Row
          ...
          class="-p-margin-top g-t-left"
        >
          <Radio-group
            v-model="unqualifiedType"
            type="button"
            @on-change="changeUnqualified()"
          >
        ...
        <search-template
          ref="searchChild"
          :option="searchOption"
          @changeSearch="getSearchInfo"
        ></search-template>

        <Row
          v-if="(radioType === 1 && unqualifiedType === 1) || radioType === 4"
          class="p-todayWork-tip"
        >
          <div class="-tip-div g-t-left">
            <Checkbox v-model="selectAllData" @on-change="changeAloneSelect"
              >全选所有作业</Checkbox
            >
          </div>
      ...
      <Table
        :loading="isFetching"
        :columns="columsType[radioType]"
        :data="dataList"
        ref="selection"
        @on-selection-change="changeSelectData"
      ></Table>
      ...
      >
        <Form ref="addInfo" :model="addInfo" :label-width="100">
          ...
          <FormItem
            label="批改模式"
            v-if="addInfo.isPassed === 1 && addInfo.homeworkType === 2"
          >
          ...
          >
            <upload-img-multiple
              v-model="addInfo.replyImgUpload"
              :option="uploadOption"
            ></upload-img-multiple>
          </FormItem>
          <FormItem label="批改音频" v-if="addInfo.isPassed === 1">
            <upload-audio
              ref="childAudio"
              v-model="addInfo.replyAudio"
              :option="uploadAudioOption"
              @otherAudioInfo="getAudioInfo"
            ></upload-audio>
          </FormItem>
          <FormItem
            label="批改文案"
            :class="{ 'ivu-form-item-required': addInfo.isPassed === 0 }"
          >
            <Input
              type="textarea"
              :rows="5"
              v-model="addInfo.replyText"
              placeholder="请输入批改文案"
            ></Input>
          </FormItem>
          <FormItem
            label="综合评分"
            v-if="addInfo.isPassed === 1"
            :class="{ 'ivu-form-item-required': addInfo.isPassed === 1 }"
          >
            <p class="-c-tips">
              此评分主要用于其他老师在查看作业记录时，能快速的对用户的作业情况有一个大致的了解，用户不可见
            </p>
            <div class="p-todayWork-score">
              <div
                class="p-todayWork-scoreItem"
                v-for="(item, index) of addInfo.scores"
                :key="index"
              >
                <span>{{ item.name }}</span>
                <InputNumber
      ...
      <examine-modal
        v-model="isOpenModalAuit"
        :dataInfo="detailInfo"
        @successAudit="successAudit"
      ></examine-modal>

      <job-record-template
        v-model="isOpenJobRecord"
        :dataInfo="recordInfo"
      ></job-record-template>

      <look-user-info
        v-model="isOpenUserInfo"
        :dataInfo="detailInfo"
      ></look-user-info>

      <job-require-template
        v-model="isOpenJobRequire"
        :dataInfo="requireInfo"
      ></job-require-template>
    </Card>
  </div>
</template>
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              ...
              params.row.homeworkType == "2"
                ? params.row.workImgSrc.map((item, index) => {
                    return h(
                      "div",
                      {
                        style: {
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              dayjs(+params.row.submitTime).format("YYYY-MM-DD HH:mm")
            );
          },
          align: "center"
        },
      ...
      columnsTwo: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              dayjs(+params.row.replyTime).format("YYYY-MM-DD HH:mm")
            );
          },
          align: "center"
        },
  ...
  methods: {
      ...
      let tipText =
        this.radioType === 4
          ? `确认将${
              this.selectAllData
                ? "所有"
                : `选中的${this.selectUserList.length}份`
            }作业移入待批改？`
          : `确认向${
              this.selectAllData
                ? "所有"
                : `选中的${this.selectUserList.length}位`
            }用户发送微信消息和短信？`;
      this.$Modal.confirm({
        title: "提示",
        content: tipText,
    ...
    getTeacherRemind() {
      this.$api.jsdJob.getTeacherRemind().then(response => {
      ...
      this.$api.jsdJob
        ...
        .then(response => {
          ...
          _self.addInfo.workImgSrc = _self.addInfo.workImgSrc
            ...
            : [];
          _self.addInfo.replyImgTmp = _self.addInfo.replyImgTmp
            ? _self.addInfo.replyImgTmp.split(",")
            : [];
          _self.addInfo.replyImgUpload = _self.addInfo.replyImgUpload
            ? _self.addInfo.replyImgUpload.split(",")
            : [];
    ...
    getList(num) {
      ...
      let params = {
        ...
        courseId: this.searchInfo.appId,
        evaluation:
          this.searchInfo.evaluation == "-1" ? "" : this.searchInfo.evaluation,
        payed: this.searchInfo.pay == "-1" ? "" : this.searchInfo.pay,
        evaluationed:
          this.searchInfo.evaluationed == "-1"
            ? ""
            : this.searchInfo.evaluationed,
        hasComment:
          this.searchInfo.hasComment == "-1" ? "" : this.searchInfo.hasComment,
        hmBegin: this.searchInfo.getStartTime
          ? new Date(this.searchInfo.getStartTime).getTime()
          : "",
        hmEnd: this.searchInfo.getEndTime
          ? new Date(this.searchInfo.getEndTime).getTime()
          : "",
    ...
    submitInfo(name) {
      ...
      this.addInfo.replyImg =
        this.corType === 1
          ? this.addInfo.replyImgTmp
          : this.addInfo.replyImgUpload;
