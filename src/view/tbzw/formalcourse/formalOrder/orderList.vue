        ...
        <Col :span="4" class="g-t-left">
          <div class="g-flex-a-j-center">
            ...
            <Select v-model="searchInfo.orderMode" @on-change="selectChange" class="-search-selectOne">
              <Option v-for="(item,index) in orderModeList" :label="item.name" :value="item.id" :key="index"></Option>
            </Select>
          </div>
        </Col>
  ...
  export default {
    ...
    methods: {
      ...
      toExcel() {
        ...
        let downUrl = `${getBaseUrl()}/compositionv3/order/getOrderExcele?startTime=${params.startTime}&endTime=${params.endTime}&payStatus=${params.payStatus}&nickName=${params.nickname}&id=${params.id}&phone=${params.phone}&orderMode=${params.orderMode}&type=${params.type}&courseId=${params.courseId=='-1' ? '' : params.courseId}`
