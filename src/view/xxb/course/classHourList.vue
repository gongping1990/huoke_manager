...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeTryOut(data) {
        this.$api.xxbTextlesson.updatelisten({
      ...
      getList(num) {
        ...
        this.$api.xxbTextlesson.pageTextLessonDetail({
      ...
      delItem(param) {
        this.$Modal.confirm({
          ...
          content: '确认要删除？',
          onOk: () => {
            this.$api.xxbTextlesson.removeTextLesson({
      ...
      submitInfo(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            let paramsUrl = this.addInfo.lessonId ? this.$api.xxbTextlesson.updateTextLesson({
              id: this.addInfo.lessonId,
              ...paramsData
            }) : this.$api.xxbTextlesson.addTextLesson(paramsData)
