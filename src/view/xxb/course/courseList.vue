...
<script>
  ...
  import xxb_courseTemplate from "./courseTemplate";
  import {forEach} from "../../../libs/tools";
  import UploadImgMultiple from "../../../components/uploadImgMultiple";

  export default {
    name: 'xxb_courseList',
    components: {UploadImgMultiple, xxb_courseTemplate, UploadImg, DatePickerTemplate},
    data() {
      return {
        tab: {
          page: 1,
    ...
    methods: {
      ...
      toJump(data) {
        this.$router.push({
          name: data.type ? 'xxb_lessonList' : 'xxb_classHourList_direct',
          query: {
            tbookId: data.id
          }
        })
      ...
      getActivityByTbookId(data) {
        this.addInfo = {}
        this.$api.xxbActivity.getActivityByTbookId({
          tbookId: data.id
        })
          .then(response => {
      ...
      getList(num) {
        ...
        this.$api.xxbTextbook.pageTextBookByQuery({
      ...
      delItem(param) {
        this.$Modal.confirm({
          ...
          content: '确认要删除？',
          onOk: () => {
            this.$api.xxbTextbook.removeTextBook({
              id: param.id
            }).then(
              response => {
      ...
      lowerItem(param) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.$api.xxbTextbook.lowerShelfTextBook({
              bookId: param.id
            }).then(
              response => {
                if (response.data.code == "200") {
      ...
      submitInfoTypeOne(name) {
        ...
        let paramsUrl = this.addInfo.id ? this.$api.xxbTextbook.updateTextBook({
          id: this.addInfo.id,
          ...paramsData
        }) : this.$api.xxbTextbook.addTextBook(paramsData)
      ...
      submitInfoTypeTwo(name) {
        ...
        let paramsUrl = this.addInfo.id ? this.$api.xxbActivity.uptNewActivity({
          id: this.addInfo.id,
          ...paramsData
        }) : this.$api.xxbActivity.saveNewActivit(paramsData)
      ...
      submitInfoTypeThree(name) {
        ...
        this.$api.xxbTextbook.editSuggestedBook({
