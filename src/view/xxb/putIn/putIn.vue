  ...
  export default {
    ...
    methods: {
      ...
      getDetailList(data) {
        ...
        this.$api.xxbInvestmanage.pageWxSubscribeKfMsgData({
      ...
      getInvestManageById(data) {
        this.$api.xxbInvestmanage.getInvestManageById({
          investId: data.id
        }).then(response => {
      ...
      listBizSystem() {
        this.$api.xxbInvestmanage.listBizSystem()
          .then(response => {
      ...
      getList(num) {
        ...
        this.$api.xxbInvestmanage.pageInvestManage({
      ...
      delItem(param) {
        this.$Modal.confirm({
          ...
          content: '确认要关闭吗？',
          onOk: () => {
            this.$api.xxbInvestmanage.finishInvest({
              investId: param.id
            }).then(
              response => {
      ...
      submitInfo(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            let paramsUrl = this.addInfo.id ? this.$api.xxbInvestmanage.updateInvestManage({
              id: this.addInfo.id,
              ...paramsData
            }) : this.$api.xxbInvestmanage.addInvestManage(paramsData)
            paramsUrl
              .then(
                response => {
                  if (response.data.code == '200') {
