  ...
  export default {
    ...
    methods: {
      ...
      getSubjectList() {
        ...
        this.$api.xxbCourse.teachSubjectList()
          .then(
            response => {
      ...
      pageByActivityLog() {
        this.$api.xxbActivity.pageByActivityLog({
      ...
      getActivityById() {
        ...
        this.$api.xxbActivity.getActivityById()
          .then(
            response => {
      ...
      getEditionList() {
        this.$api.xxbCourse.editionList()
          .then(
            response => {
      ...
      submitInfo(name) {
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.isSending = true
            let promiseDate = this.addInfo.id ? this.$api.xxbBook.updateTeaching(this.addInfo) : this.$api.xxbBook.addTeaching(this.addInfo)
            promiseDate
              .then(
                response => {
                  if (response.data.code == '200') {
      ...
      submitInfoActive() {
        ...
        this.isSending = true
        let promiseDate = this.activeInfo.id ? this.$api.xxbActivity.uptActivity({
          ...this.activeInfo,
        ...
        }) : this.$api.xxbActivity.saveActivity({
          ...this.activeInfo,
          ddgPrice: this.activeInfo.ddgPrice*100,
          oriPrice: this.activeInfo.oriPrice*100,
          activityPrice: this.activeInfo.activityPrice*100
