    ...
    methods: {
      ...
      delItem(param, num) {
        this.$Modal.confirm({
          ...
          content: '确认要删除吗？',
          onOk: () => {
            let delInterface = num == '1' ? this.$api.xxbBook.delChapter({
              id: param
            }) : this.$api.xxbBook.delLesson({
              id: param
            })
            delInterface
              .then(
      ...
      submitInfo(name) {
        ...
        if (this.rootNode.num == '1') {
          promiseDate = this.addInfo.id ? this.$api.xxbBook.updateChapter(this.addInfo) : this.$api.xxbBook.addChapter(this.addInfo)
        } else {
          this.addInfo.pinyin = this.pinyinInfo
          this.addInfo.chapterId = this.addInfo.id ? this.rootNode.chapterId : this.rootNode.id
          promiseDate = this.addInfo.id ? this.$api.xxbBook.updateLesson(this.addInfo) : this.$api.xxbBook.addLesson(this.addInfo)
        }
